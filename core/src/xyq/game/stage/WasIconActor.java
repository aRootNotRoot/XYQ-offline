...
import xyq.system.assets.WasIcon;

/**
 * 自定义演员（绘制时处理 位置，尺寸，缩放比，旋转角度 和 color/alpha 等属性）
 */
public class WasIconActor extends Actor {

    public WasIcon img;
    float drawOffsetX;
	float drawOffsetY;
    public WasIconActor(ResSystem RS,String pack,String was) {
        super();
        this.img = new WasIcon(RS,pack,was,null);
        setSize(this.img.getW(), this.img.getH());
    }
    public WasIconActor(ResSystem RS,String pack,String was,PPCData pp) {
        super();
        this.img = new WasIcon(RS,pack,was,pp);
        setSize(this.img.getW(), this.img.getH());
    }
    public WasIcon getImg() {
        return img;
    }

    public void setImg(WasIcon region) {
        this.img = region;
        setSize(this.img.getW(), this.img.getH());
    }
