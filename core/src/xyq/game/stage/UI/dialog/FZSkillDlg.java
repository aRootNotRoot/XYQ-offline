...
public class FZSkillDlg extends Group{
	XYQGame game;
	...
	private FZSkillDlg me;
	
	public FZSkillDlg(final XYQGame game){
		super();
		this.game=game;
		me=this;
		setSize(451, 465);
		
		panel=new WasActor(game.rs,"wzife.wdf","UI\\辅助技能");
	...
	void loadLearnedFZ() {
		String menpai="辅助";
		ArrayList<SkillData> learnedMagics=game.db.loadLearnedFZSkill();
		learnedSkillIcons=new WasIconActor[14];
		if(learnedMagics.size()==0){
			if(game.is_Debug){
				Gdx.app.log("[ XYQ ]", "[辅助技能对话框] -> 当前角色未学习任何辅助技能，不用加载已经学会的技能");
			}
			return;
		}
